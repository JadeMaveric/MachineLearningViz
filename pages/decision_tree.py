...
def run(st):
    global total_count, labeled_count, node_count, rules, nodes_processed
    st.title("DecisionTree Visualiser")
    st.subheader("By [@JuliusAlphonso](https://twitter.com/JuliusAlphonso) | Fork/Star on [GitHub](https://github.com/JadeMaveric/MachineLearningViz)")
    ...
    def format_data(dataset):
        ...
        df[targ_cols] = df[targ_cols].apply(
            lambda x: True if x.lower() in ('positive', 'yes', True) else False
        )
        return df
